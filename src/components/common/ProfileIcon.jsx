...
import { deleteProfilePicture, updateProfilePicture } from "../../api/authApi";
...
const getStoredUser = () => {
  try {
    ...
    if (
      ...
      fallbackUser.employee_id === currentEmployeeId
    ) {
      return fallbackUser;
    }
...
const saveUserForCurrentEmployee = (userData) => {
  const employeeId =
    localStorage.getItem("employee_id") || userData?.employee_id || "";
...
const normalizeProfilePictureUrl = (value) => {
  ...
  if (
    trimmedValue.startsWith("http://66.116.207.88/") &&
    !trimmedValue.includes(":14250")
  ) {
...
const ProfileIcon = () => {
  ...
  const storedEmployeeId =
    userData?.employee_id || localStorage.getItem("employee_id") || "";
  ...
  const handleProfileUpdate = (updatedData) => {
    setUserData((prev) => {
      ...
      if (merged.profile_picture) {
        merged.profile_picture = normalizeProfilePictureUrl(
  ...
  const handleProfilePhotoChange = async (event) => {
    ...
    try {
      ...
      const finalImageUrl =
        normalizeProfilePictureUrl(rawImageUrl) || localPreview;
      ...
      alert(
        error?.response?.data?.detail || "Unable to upload profile picture.",
      );
    } finally {
      setUploading(false);
      event.target.value = "";
  ...
  const handleRemoveProfilePhoto = async () => {
    try {
      ...
      alert(
        error?.response?.data?.detail || "Unable to remove profile picture.",
      );
    } finally {
      setUploading(false);
      setOpen(false);
