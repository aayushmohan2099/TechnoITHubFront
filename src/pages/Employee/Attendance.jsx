...
const Attendance = () => {
    ...
    if (
      ...
      const updateTimer = () => {
        ...
        const seconds = Math.max(
          0,
          Math.floor((currentTime - punchInTime) / 1000),
        );
  ...
  const handlePunch = async () => {
    ...
    try {
      ...
      if (
        attendancePayload?.status === "Currently Working" ||
        (attendancePayload?.punch_in_time && !attendancePayload?.punch_out_time)
      ) {
        setAttendance({
          status: "Currently Working",
          punchInTime:
            attendancePayload.punch_in_time || attendancePayload.punch_in,
          punchOutTime: null,
          totalSeconds: 0,
        });
        setElapsedSeconds(0);
        setSuccessMessage(
          attendancePayload.message ||
            data?.message ||
      ...
      if (
        attendancePayload?.punch_out_time ||
        attendancePayload?.punch_out ||
        attendancePayload?.status === "Completed"
      ) {
        if (timerRef.current) {
          clearInterval(timerRef.current);
          timerRef.current = null;
        ...
        const outTime =
          attendancePayload.punch_out_time || attendancePayload.punch_out;
        const totalSec = Number(attendancePayload.total_seconds || 0);

        setAttendance({
          status: "Completed",
          punchInTime:
            attendancePayload.punch_in_time ||
            attendancePayload.punch_in ||
            attendance.punchInTime,
        ...
        {attendance.status === "Completed" && (
          <div className="py-8 text-center">
            ...
            <p className="mt-3 text-sm font-medium text-blue-600">
              Attendance is completed for this date
            </p>
          </div>
        )}
