...
const Attendance = () => {
  ...
  const fetchAttendance = async (pageNumber = page, dateValue = filterDate) => {
    try {
      ...
      setError(
        err?.response?.data?.detail || "Failed to load attendance history.",
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      setEmployeeHistory(
        employeeRecords.sort(
          (a, b) =>
            new Date(b.attendance_date || 0) - new Date(a.attendance_date || 0),
        ),
      );
    } catch (err) {
  ...
  const employeeRecords = selectedEmployee
    ? employeeHistory.filter(
        (item) =>
          String(item.employee_id) === String(selectedEmployee.employee_id),
      )
    : [];
  ...
  const getAttendanceForDate = (date) => {
    return employeeRecords.find((item) => item.attendance_date === date);
  };
  ...
  const handleViewEmployee = (item) => {
    ...
    setSelectedDate(item.attendance_date);

    setCurrentMonth(new Date(`${item.attendance_date}T00:00:00`));
  ...
  return (
    <div className="min-h-full bg-gray-50 p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => fetchAttendance(page, filterDate)}
          disabled={loading}
        ...
        {loading ? (
          ...
          <div className="py-14 text-center">
            ...
            <p className="mt-1 text-xs text-gray-400">
              No attendance is available for {formatDate(filterDate)}.
            </p>
          </div>
        ) : (
                ...
                {Array.from({
                  ...
                  return (
                    <button
                      ...
                      className={`min-h-[80px] border-b border-r p-2 text-left transition ${
                    ...
                    <div className="mt-6 space-y-4">
                      ...
                      <div className="rounded-lg bg-white p-4">
                        ...
                        <p className="mt-1 text-sm font-semibold text-gray-800">
                          {formatTime(selectedDateAttendance.punch_in)}
                        </p>
                      </div>
                      ...
                      <div className="rounded-lg bg-white p-4">
                        ...
                        <p className="mt-1 text-sm font-semibold text-gray-800">
                          {selectedDateAttendance.punch_out
                            ? formatTime(selectedDateAttendance.punch_out)
                            : "Not Punched Out"}
                        </p>
                      </div>
                      ...
                      <div className="rounded-lg bg-white p-4">
                        ...
                        <p className="mt-1 text-lg font-semibold text-ettm-blue">
                          {formatDuration(selectedDateAttendance.total_seconds)}
                        </p>
                      </div>
                ...
                <div className="max-h-80 space-y-2 overflow-y-auto">
                  {employeeRecords.map((record, index) => (
                    <button
                      key={
                        record.id ||
                        `${record.employee_id}-${record.attendance_date}-${index}`
                      }
                      type="button"
                      onClick={() => setSelectedDate(record.attendance_date)}
                      className={`flex w-full items-center justify-between rounded-lg border p-3 text-left transition ${selectedDate === record.attendance_date ? "border-ettm-blue bg-blue-50" : "border-gray-200 bg-gray-50 hover:bg-gray-100"}`}
                    >
                      <div>
