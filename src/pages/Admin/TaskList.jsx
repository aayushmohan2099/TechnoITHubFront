...
import { getAllTasks, updateTask, deleteTask } from "../../api/employeeApi";
...
const SortHeader = ({ label, field, ordering, onSort }) => (
...
const TaskList = () => {
  ...
  const fetchTasks = useCallback(
    async (pageNumber = 1, filters = EMPTY_FILTERS) => {
      try {
        setLoading(true);
        setErrorMessage("");
        ...
        if (Array.isArray(response)) {
          ...
          setTotalCount(
            response?.count ?? response?.data?.count ?? results.length,
          );

          setNextPage(response?.next ?? response?.data?.next ?? null);

          setPreviousPage(
            response?.previous ?? response?.data?.previous ?? null,
          );
        }
      } catch (error) {
  ...
  const startItem = tasks.length === 0 ? 0 : (page - 1) * PAGE_SIZE + 1;

  const endItem = tasks.length === 0 ? 0 : startItem + tasks.length - 1;
  ...
  const handleEditTask = () => {
    ...
    setEditForm({
      ...
      start_date: selectedTask.start_date?.slice(0, 10) || "",
  ...
  const validateEditForm = () => {
    ...
    if (
      editForm.start_date &&
      editForm.deadline &&
      editForm.deadline < editForm.start_date
    ) {
  ...
  const handleUpdateTask = async () => {
    ...
    try {
      ...
      await fetchTasks(page, appliedFilters);
  ...
  const handleDeleteTask = async () => {
    ...
    try {
      ...
      await fetchTasks(targetPage, appliedFilters);
  ...
  const columns = [
    {
      key: "serial_number",
      label: "Sr. No.",
      render: (_task, index) => (page - 1) * PAGE_SIZE + index + 1,
    },
    {
      key: "title",
      label: "Task",
    ...
    },
    {
      key: "created_by_name",
      label: "Created By",
      render: (task) => task.created_by_name || "-",
    },
    {
      key: "start_date",
      label: (
      ...
      render: (task) => formatDate(task.start_date),
    },
    {
      key: "deadline",
      label: (
  ...
  const dailyUpdates = selectedTask?.daily_updates || [];
  ...
  return (
    <div className="min-h-full bg-gray-50 p-6">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-ettm-blue">Task List</h1>

          <p className="mt-1 text-sm text-gray-500">
            View, edit and manage tasks assigned to employees.
          </p>
        </div>
        ...
        <button
          type="button"
          disabled={loading}
          onClick={() => fetchTasks(page, appliedFilters)}
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Search by task, employee name, or employee ID.
            </p>
          </div>
          ...
          <div className="flex items-center justify-between border-t border-gray-200 px-5 py-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={() => fetchTasks(page - 1, appliedFilters)}
        ...
        {selectedTask &&
          (isEditing ? (
            <div className="space-y-5">
              <FormField label="Task Title *" error={editErrors.title}>
                <input
                  type="text"
                  name="title"
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <FormField label="Start Date *" error={editErrors.start_date}>
                  <input
                    type="date"
                    name="start_date"
                ...
                <FormField label="Deadline *" error={editErrors.deadline}>
                  <input
                    type="date"
                    name="deadline"
                ...
                <FormField label="Priority *" error={editErrors.priority}>
                  <select
                    name="priority"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4 rounded-xl bg-gray-50 p-4 sm:grid-cols-2">
                <DetailItem
                  label="Assigned Employee"
                  value={selectedTask.assigned_to_name || "-"}
                />

                <DetailItem
                  label="Employee ID"
                  value={selectedTask.assigned_to_emp_id || "-"}
                />

                <DetailItem
                  label="Created By"
                  value={selectedTask.created_by_name || "-"}
                />
                ...
                <DetailItem
                  label="Start Date"
                  value={formatDate(selectedTask.start_date)}
                />
              ...
              <div>
                ...
                {dailyUpdates.length > 0 ? (
                  <div className="space-y-3">
                    {dailyUpdates.map((update, index) => {
                      const progress = Math.min(
                        100,
                        Math.max(0, Number(update.progress_percent || 0)),
                      );
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between gap-3">
                            <div>
                              <p className="text-sm font-semibold text-gray-800">
                                {update.task_title || `Update ${index + 1}`}
                              </p>

                              <p className="mt-1 text-xs text-gray-500">
                                Updated by: {update.employee_name || "-"}
                              </p>
                            </div>
                          ...
                          <p className="mt-4 text-sm text-gray-700">
                            {update.update_text || "No update text provided."}
                          </p>
                  ...
                  <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-5 text-center text-sm text-gray-500">
                    No daily updates have been added.
                  </div>
                )}
              </div>
